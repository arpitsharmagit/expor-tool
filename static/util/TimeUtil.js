 ...
 */
	parseUTCDate: function parseUTCDate(string) {
		...
		if (match) {
			...
			var millis = match[7];
			if (millis) {
				//convert the fractional second into milliseconds
 ...
 **/
	parseCSharpTimespan: function parseCSharpTimespan(timeStr) {
	...
	formatCSharpTimespan: function formatCSharpTimespan(timespan) {
		...
		return (isNegative ? "-" : "") + (days > 0 ? days + "." : "") + pad2(hours) + ":" + pad2(minutes) + ":" + pad2(seconds) + (fractionalSeconds > 0 ? fractionalSeconds.slice(fractionalSeconds.indexOf(".")) : "");
	...
	formatTimespan: function formatTimespan(timespan, full) {
  ...
  */
	formatMacAddressNumbersOnly: function formatMacAddressNumbersOnly(macAddress) {
